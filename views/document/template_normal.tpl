...
**Windows 教程:** [https://github.com/aydcyhr/ADY-wiki/blob/master/README_WIN.md](docs/README_WIN.md)

**Linux  教程:**  [https://github.com/aydcyhr/ADY-wiki/blob/master/README_LINUX.md](docs/README_LINUX.md)

**PDF 导出配置教程**  [https://github.com/aydcyhr/ADY-wiki/blob/master/docs/README_LINUX.md](docs/WKHTMLTOPDF.md)
...
ADY-wiki 使用MySQL储存数据，且编码必须是`utf8mb4_general_ci`。请在安装前，把数据库配置填充到项目目录下的 conf/app.conf 中。

如果conf目录下不存在 app.conf 请重命名 app.conf.example 为 app.conf。

如果 ADY-wiki 根目录下存在 install.lock 文件表示已经初始化过数据库，想要重新初始化数据库，只需要删除该文件重新启动程序即可。
...
```bash
docker run -p 8181:8181 -e MYSQL_PORT_3306_TCP_ADDR=127.0.0.1 -e MYSQL_PORT_3306_TCP_PORT=3306 -e MYSQL_INSTANCE_NAME=mindoc_db -e MYSQL_USERNAME=root -e MYSQL_PASSWORD=123456 -e httpport=8181 -d daocloud.io/aydcyhr/ADY-wiki:latest
```
...
![项目设置](https://raw.githubusercontent.com/aydcyhr/ADY-wiki/master/uploads/20170501203656.png)
...
![超级管理员后台](https://raw.githubusercontent.com/aydcyhr/ADY-wiki/master/uploads/20170501204710.png)
