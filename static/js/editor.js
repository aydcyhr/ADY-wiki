/**
 * Created by aydcyhr on 2017/4/29 0029.
 */
...
function openLastSelectedNode() {
    ...
    if (window.localStorage) {
        var $selectedNodeId = window.sessionStorage.getItem("ADY-wiki::LastLoadDocument:" + window.book.identify);
        try {
            if ($selectedNodeId) {
                //遍历文档树判断是否存在节点
...
function setLastSelectNode($node) {
    if (window.localStorage) {
        if (typeof $node === "undefined" || !$node) {
            window.sessionStorage.removeItem("ADY-wiki::LastLoadDocument:" + window.book.identify);
        } else {
            var nodeId = $node.id ? $node.id : $node.node.id;
            window.sessionStorage.setItem("ADY-wiki::LastLoadDocument:" + window.book.identify, nodeId);
        }
    }
}
...
$("#addDocumentModal").on("hidden.bs.modal", function () {
    $(this).find("form").html(window.sessionStorage.getItem("ADY-wiki::addDocumentModal"));
    var $then = $("#addDocumentModal");
    ...
    window.sessionStorage.setItem("ADY-wiki::addDocumentModal", $(this).find("form").html())
});
